...
@app.patch("/posts/{post_id}/likes/{is_liked}", response_model=int)
def update_post_likes(post_id: int, is_liked: int, db: Session = Depends(get_db)):
    try:
        return crud.update_likes(db=db, post_id=post_id, is_liked=is_liked)
